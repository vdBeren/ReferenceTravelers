...
class RTGameOverScene: SKScene {
    ...
    func initGameOver(){
        ...
        skeleton!.position = CGPoint(x: self.frame.maxX / 2, y: self.frame.maxY / 5.5)
        skeleton!.alpha = 0
        addChild(skeleton!)
        ...
        player!.position = CGPoint(x: self.frame.maxX / 2.25, y: self.frame.maxY / 5)
        addChild(player!)
        
        tap = RTLabelText(text: "Tap to continue", fontSize: 40, minimum: 15)
        tap!.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
        addChild(tap!)
        
        gameLabel = RTLabelText(text: "GAME", fontSize: 100, minimum: 4)
        gameLabel!.position = CGPoint(x: self.frame.maxX / 2.7, y: self.size.height/1.5)
        addChild(gameLabel!)
        
        overLabel = RTLabelText(text: "OVER", fontSize: 100, minimum: 4)
        overLabel!.position = CGPoint(x: self.frame.maxX / 1.5, y: self.size.height/1.5)
        addChild(overLabel!)
